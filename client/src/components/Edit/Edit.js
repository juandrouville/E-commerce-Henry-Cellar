...
export default function Edit({id}) {
    ...
    useEffect(()=>{
        let arr=[]
        if(productDetail.categories)productDetail.categories.map(category=>arr.push(category.name))
        setState({
        ...
        <div>
             <form onSubmit={handleSubmit}>
                ...
                <div>
                    ...
                    <textarea type="text" name="description" defaultValue={productDetail.description} onChange={handleChange}/>
                    {errors.description && <p className="danger">{errors.description}</p>}
                </div>
                <div>
                    <label>Price:</label>
                    <input type="number" min={0} name="price" defaultValue={productDetail.price} onChange={handleChange}/>
                    {errors.price && <p className="danger">{errors.price}</p>}
                </div>
                <div>
                    <label>Image(URL):</label>
                    <input type="text" name="image" defaultValue={productDetail.image} onChange={handleChange}/>
                    {errors.image && <p className="danger">{errors.image}</p>}
                </div>
               <div>
                   <label>Stock:</label>
                   <input type="number" min={0} name="stock" defaultValue={productDetail.stock} onChange={handleChange}/>
